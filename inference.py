...
if __name__=='__main__':
    ...
    df = pd.read_csv(r"D:\Dataset\jigsaw\toxic_folds.csv")
    ...
    val_sentences = pad_sequences(val_sequenceses, 200)
    ...
    model = SentimentNet(output_size, embedding_matrix, hidden_dim, n_layers)
    model.load_state_dict(torch.load('model.bin'))
    predicted = model.predict(valid_dataset, batch_size=16, device='cuda')
    df1 = pd.DataFrame(predicted)
    df1['target'] = pd.DataFrame(val_labels)
    df1.to_csv('toxic_output.csv', index=False)